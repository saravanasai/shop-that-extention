...
const App: React.FC<{}> = () => {
  ...
  const fetchProductInfo = async (productId, siteId) => {
    axios.defaults.headers.post["Content-Type"] =
      "application/json;charset=utf-8";
    axios.defaults.headers.post["Access-Control-Allow-Origin"] = "*";
    axios
      .get("http://localhost:3001/" + productId + "?siteId=" + siteId)
  ...
  useEffect(() => {
    window.document.addEventListener("mouseover", (e) => {
      ...
      if (e.target.className == "product-record__heading--text") {
        //@ts-ignore
        fetchProductInfo(String(e.target.innerHTML).trim(), 1);
      }
      //@ts-ignore
      if (e.target.className == "pt-cv-ctf-value") {
        //@ts-ignore
        fetchProductInfo(String(e.target.innerText).trim(), 2);
      }

      //@ts-ignore
      if (e.target.className == "yiq Npp") {
        //@ts-ignore
        fetchProductInfo(String(e.target.innerText).trim(), 3);
      }
    });
  }, []);
  ...
  let subTotal = Object.keys(productQuantitys).reduce((acc, key) => {
    ...
    console.log(
      "🚀 ~ file: contentScript.tsx:66 ~ subTotal ~ productData:",
      productData?.price
    );
    return (acc += Number(productData?.price) * productQuantitys[key]);
    // return 0;
  }, 0);
